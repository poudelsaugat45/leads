...
class Model
{
    ...
    {
        $sql = "SELECT lead.l_id, lead.l_name, lead.address, lead.contact, counsellor.c_id, lead.status, lead.next_followup FROM lead INNER JOIN counsellor WHERE lead.c_id = counsellor.c_id";
        $query = $this->db->prepare($sql);
        $query->execute();
    ...
    public function counsellor($c_name){

      $sql = "SELECT lead.l_name, lead.l_id, counsellor.c_name, counsellor.c_id FROM counsellor INNER JOIN lead WHERE c_name= :c_name";
      $query = $this->db->prepare($sql);
    ...
    public function customized($c_name, $semester){

      $sql = "SELECT lead.l_name, lead.l_id, lead.address, counsellor.c_name, followup.f_date, COUNT(followup.l_id)  AS no_of_followup, lead.semester  FROM lead INNER JOIN followup INNER JOIN counsellor WHERE lead.l_id= followup.l_id AND counsellor.c_id=lead.c_id AND counsellor.c_name= :c_name AND lead.semester= :semester ";
      $query = $this->db->prepare($sql);
      $parameters = array(':c_name' => $c_name, ':semester' => $semester);
    ...
    public function getCounsellorNames()
    {
      $sql = "SELECT c_name FROM counsellor";
      $query = $this->db->prepare($sql);
      $query->execute();
      // fetchAll() is the PDO method that gets all result rows, here in object-style because we defined this in
      // core/controller.php! If you prefer to get an associative array as the result, then do
